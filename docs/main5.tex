...
\midrule
\rowcolor{run1blue}
\textbf{Run 1} & Infix CoOp + Ordinal & LoRA ($W_q, W_v$, $r{=}2, \alpha{=}1.0$) & Frozen Backbone & Infix continuous prompt ($M{=}4$) + Learnable residual class tokens $\Delta e_k$ & $\mathcal{L}_{\mathrm{ce}} + \lambda_{\mathrm{ord}}\mathcal{L}_{\mathrm{ord}}$ ($\lambda_{\mathrm{ord}}{=}1.0$) \\
\addlinespace
\rowcolor{run2orange}
...
\addlinespace
\rowcolor{run3green}
\textbf{Run 3} & CoOp-CSC Dual LoRA & LoRA ($W_q, W_v$, $r{=}2, \alpha{=}1.0$) & LoRA ($W_q, W_v$, $r{=}2, \alpha{=}1.0$) & Class-Specific Context ($K \times M \times d$ continuous prompt vectors) & Standard $\mathcal{L}_{\mathrm{ce}}$\textsuperscript{*} \\
\addlinespace
\rowcolor{run4red}
\textbf{Run 4} & Plain LoRA + ResCls & LoRA ($W_q, W_v$, $r{=}2, \alpha{=}1.0$) & LoRA ($W_q, W_v$, $r{=}2, \alpha{=}1.0$) & Handcrafted natural template + Learnable residual class tokens $\delta_k$ & $\mathcal{L}_{\mathrm{ce}}$ / $(\mathcal{L}_{\mathrm{ce}} + \lambda_{\mathrm{ord}}\mathcal{L}_{\mathrm{ord}})^{\dagger}$ \\
\bottomrule
\end{tabularx}
\vspace{0.3em}
\raggedright\small \textit{Note: ViT-B/16 operates with vision dimension $d_v = 768$ (projected to 512) and text dimension $d_t = 512$. All LoRA modules inject into attention projection matrices with rank $r=2$ and scaling factor $\alpha=1.0$. \textsuperscript{*}For Run 3, ordinal loss was planned but the flag was omitted during benchmark execution, defaulting to standard $\mathcal{L}_{\mathrm{ce}}$. $^{\dagger}$Run 4 was evaluated both without ordinal loss ($\mathcal{L}_{\mathrm{ce}}$) and with ordinal penalty ($\lambda_{\mathrm{ord}}{=}2.0$).}
\end{table}
...
criterion = CompositeCriterion(
    ...
    cost_matrix=cost_matrix, lambda_ord=1.0
)
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{Loss Function}: Standard cross-entropy loss $\mathcal{L}_{\mathrm{ce}}$ (ordinal loss was originally planned for Run~3, but the \path{--use_ordinal} flag was omitted during benchmark execution).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Loss Function}: Evaluated under both standard cross-entropy classification ($\mathcal{L}_{\mathrm{ce}}$, without ordinal) and joint ordinal cost loss ($\mathcal{L}_{\mathrm{ce}} + \lambda_{\mathrm{ord}}\mathcal{L}_{\mathrm{ord}}$ with $\lambda_{\mathrm{ord}} = 2.0$).
\end{enumerate}
...
\midrule
Loss Formulation & $\mathcal{L}_{\mathrm{ce}} + \lambda_{\mathrm{ord}}\mathcal{L}_{\mathrm{ord}}$ & $\mathcal{L}_{\mathrm{ce}}$ & $\mathcal{L}_{\mathrm{ce}}$\textsuperscript{*} & $\mathcal{L}_{\mathrm{ce}}$ / $(\mathcal{L}_{\mathrm{ce}} + \lambda\mathcal{L}_{\mathrm{ord}})^{\dagger}$ & \path{--use_ordinal} \\
Ordinal Weight ($\lambda_{\mathrm{ord}}$) & $1.0$ & N/A & Omitted\textsuperscript{*} & None / $2.0^{\dagger}$ & \path{--lambda_ord} \\
Cost Matrix Metric & Expected Cost & N/A & N/A & Expected Cost$^{\dagger}$ & \path{--ordinal_loss_type} \\
...
\end{tabularx}
\vspace{0.3em}
\raggedright\small \textit{\textsuperscript{*}For Run 3, ordinal loss was planned but the \path{--use_ordinal} flag was omitted during benchmark execution, defaulting to standard $\mathcal{L}_{\mathrm{ce}}$. $^{\dagger}$For Run 4, experiments were evaluated both without ordinal loss ($\mathcal{L}_{\mathrm{ce}}$) and with ordinal penalty ($\lambda_{\mathrm{ord}} = 2.0$).}
\end{table}
