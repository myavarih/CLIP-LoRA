...
def save_lora(args, list_lora_layers, filename=None):
    weights = {}
...
def load_lora(args, list_lora_layers, filename=None):
    # to manage names like ViT-B/16
    backbone = args.backbone.replace('/', '').replace('-', '').lower()
    if filename is None:
        filename = args.filename
    load_path = f'{args.save_path}/{backbone}/{args.dataset}/{args.shots}shots/seed{args.seed}/{filename}.pt'
