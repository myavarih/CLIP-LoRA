...
we needed to analyze the ablation results and write a LaTeX document: so created a python script to generate plots and updated main.tex with the analysis.
we needed a deep dive on the 4-shot baseline: so expanded the LaTeX document to include zero-shot vs final comparisons for PCA, t-SNE, predictions, and attention.
the user requested a less formal, more student-like tone: so rewrote the main.tex document with shorter sentences, human-like hypotheses, and a casual lab report style.
we needed to include the second phase of ablations in the lab report: so updated docs/generate_plots.py to point to results(2), generated new visualization comparisons, and expanded docs/main.tex with a new section on advanced ablations including dropout, alpha scaling, and modality targeting.
