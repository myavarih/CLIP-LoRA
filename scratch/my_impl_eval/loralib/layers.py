...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.to(device=existing_linear.weight.device, dtype=existing_linear.weight.dtype)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.dropout is None: # do as before
            if self.r > 0 and not self.merged:
                ...
                return nn.Linear.forward(self, x, **kwargs).to(orig_dtype)
        ...
        if self.r > 0 and not self.merged:
            lora_adjustment = torch.matmul(x, self.merge_BA('weight').transpose(0, 1).to(self.weight.dtype)) * self.scaling 
...
class Conv1d(nn.Conv1d, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            ...
            return nn.Conv1d.forward(self, x, **kwargs).to(orig_dtype)
...
class Conv2d(nn.Conv2d, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            ...
            return nn.Conv2d.forward(self, x, **kwargs).to(orig_dtype)
...
class Conv3d(nn.Conv3d, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            ...
            return nn.Conv3d.forward(self, x, **kwargs).to(orig_dtype)
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        target_dtype = existing_mha.in_proj_weight.dtype
        target_device = existing_mha.in_proj_weight.device

        #self.qkv = nn.Linear(self.embed_dim, self.embed_dim * 3, bias=existing_mha.in_proj_bias is not None)
        self.q_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).to(device=target_device, dtype=target_dtype)
        self.k_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).to(device=target_device, dtype=target_dtype)
        self.v_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).to(device=target_device, dtype=target_dtype)
        self.proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.out_proj.bias is not None).to(device=target_device, dtype=target_dtype)
